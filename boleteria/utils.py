...
def calculo_general(cant_nino, cant_adulto, cant_adu_may):
    total_nino = cant_nino * config.tarifa['boleto_nino']
    total_adulto = cant_adulto * config.tarifa['boleto_adulto']
    total_adu_may = cant_adu_may * config.tarifa['boleto_adu_may']
    total_general = total_nino + total_adulto + total_adu_may
    return total_general

def calculo_descuento(valor):
    descuento_valor = 0
    if valor >= config.descuento['monto_minimo']:
        descuento_valor = valor * config.descuento['porcentaje']
...
def generar_comprobante(total_nino, total_adulto, total_adu_may):
    ...
    with open('comprobante.txt', 'w') as archivo:
        ...
        archivo.write("---------------------\n")
        archivo.write(f"Niño: {total_nino} ${total_nino*config.tarifa['boleto_nino']}\n")
        archivo.write(f"Adulto: {total_adulto} ${total_adulto*config.tarifa['boleto_adulto']}\n")
        archivo.write(f"Adulto mayor: {total_adu_may} ${total_adu_may*config.tarifa['boleto_adu_may']}\n")
        archivo.write("---------------------\n")
